...
class SuratTugasService
{
    ...
    public function __construct()
    {
        ...
        $this->excelFilePath = public_path('files/PETA PEGAWAI SOERADJI.xlsx');
    ...
    public function getPegawaiData()
    {
        ...
        if (!file_exists($this->excelFilePath)) {
            \Log::error("File Excel pegawai tidak ditemukan di path: " . $this->excelFilePath);
            return [];
        }
        ...
        try {
            ...
            // Debug: Log jumlah data yang berhasil dibaca
            \Log::info("Berhasil membaca " . count($data) . " data pegawai dari Excel");
    ...
    {
        // Debug: Log pencarian
        \Log::info("Mencari pegawai dengan kata kunci: " . $term);
        
        if (empty($term) || strlen($term) < 2) {
            \Log::info("Pencarian dibatalkan karena kata kunci kurang dari 2 karakter");
            return [];
        }
        ...
        // Debug: Log jumlah total data pegawai
        \Log::info("Total data pegawai tersedia: " . count($pegawaiData));
        ...
        // Debug: Log jumlah hasil pencarian
        \Log::info("Ditemukan " . count($results) . " hasil pencarian untuk kata kunci: " . $term);
    ...
    public function generatePreviewHtml(array $data, int $templateType)
    {
        ...
        $placeholders = [
            ...
            // Tambahkan placeholder untuk tanda baca
            'semicolon' => ';',
            'dot' => '.',
        ];
